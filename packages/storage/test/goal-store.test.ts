...
test("GoalSnapshotCodec round-trips a complete Goal and rejects extra fields", () => {
    ...
    assert.equal(encoded.metadata.schemaVersion, 7);
...
test("GoalSnapshotCodec reads legacy v5 with a zero Trajectory boundary without rewriting it", () => {
    ...
    const { memoryProtocol: _memoryProtocol, ...legacyDefinition } = encoded.definition;
    const legacy = {
        ...encoded,
...
test("GoalSnapshotCodec round-trips bounded Action memory and approval state", () => {
    ...
    assert.equal(encodedAction.metadata.schemaVersion, 7);
...
test("GoalSnapshotV7Schema rejects invalid v7 cross-field combinations", () => {
    ...
    for (const invalidSnapshot of invalidSnapshots) {
        assert.equal(GoalSnapshotV7Schema.safeParse(invalidSnapshot).success, false);
    }
});

test("GoalSnapshotV7Schema enforces workflow and Run cross-field invariants", () => {
    const encoded = goalSnapshotCodec.encode(createSnapshot());
    const waitStep = {
        kind: "decision",
        result: {
