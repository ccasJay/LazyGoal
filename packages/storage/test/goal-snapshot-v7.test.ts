...
test("Snapshot v7 round-trips structured protocol and Memory revision", () => {
...
test("Snapshot v7 persists checkpoint protocol without a revision and preserves legacy access", () => {
...
test("v5 and v6 decode as read-only legacy and upgrade only on the next encode", () => {
    ...
    const { memoryProtocol: _protocol, ...legacyDefinition } = encoded.definition;
    ...
    for (const legacy of [v5, v6]) {
        ...
        assert.equal(goalSnapshotCodec.encode(restored).metadata.schemaVersion, 7);
    }
});

test("Snapshot v7 rejects unknown protocols, corrupt revisions, and cross-protocol fields", () => {
    ...
    for (const invalid of invalidSnapshots) {
        assert.equal(GoalSnapshotV7Schema.safeParse(invalid).success, false);
