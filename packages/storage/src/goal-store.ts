...
import type { GoalSnapshotV4 } from "./goal-snapshot";
 ...
 *
 * @remarks
 * `save` 先将 Goal 编码为严格 v4 Snapshot，`restore` 再解码回 Runtime
...
export class InMemoryGoalStore implements GoalStore {
    private readonly snapshots = new Map<string, GoalSnapshotV4>();
 ...
 * 经 Codec 编码并校验严格 v4 协议；恢复时解码，v1 至 v3 与
 * 未知版本统一抛出 {@link GoalSnapshotProtocolError}，读取失败绝不触发
 * 写回，之后显式保存恢复结果时才以 v4 原子替换。
 *
...
export class JsonFileGoalStore implements GoalStore, GoalCatalog {
     ...
     *
     * @returns 文件不存在时返回 `undefined`，否则返回与存储隔离的 v4 Goal。
