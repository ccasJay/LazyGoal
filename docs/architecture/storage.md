...
`@lazygoal/storage` 拥有持久化文件表示与实现：Profile 文件 DTO、独立 v5/v6/v7/v8/v9 Goal Snapshot DTO 与严格 Schema、Runtime↔Snapshot 双向 Codec、Trajectory/Diagnostic JSONL Store，以及实现 Runtime/Agent Port 的内存/JSON 文件 Store。它依赖 Runtime 的 Port 与领域契约；Runtime 不反向加载本模块。
...
| [GoalSnapshotV5/V6/V7/V8/V9 协议](../../packages/storage/src/goal-snapshot.ts) | 独立 v5/v6/v7/v8/v9 Snapshot DTO、严格字段与跨字段不变量校验；v6+ 的 `committedThroughSequence` 建立 Trajectory 恢复边界，v7+ 保存 Memory 协议与 revision 指针，v8 保存 Model Context 协议，v9 保存 Retrieval 协议 | 文件系统 I/O、构造 Runtime Goal、版本迁移 |
| [GoalSnapshotCodec](../../packages/storage/src/goal-snapshot-codec.ts) | Runtime Goal↔v9 Snapshot 的 encode/decode 深复制转换；decode 接受 v5/v6/v7/v8/v9，旧版本按 legacy `checkpoint@1`、`conversation@1`、`none@1` 恢复，下一次保存升级为 v9 | 文件系统 I/O、读写 Store |
...
Goal 快照统一经 `GoalSnapshotCodec`：`save` 先对 Runtime Goal 按同一严格 v9 Schema 校验（拒绝多余字段、非法 StepRecord、缺失或非正整数的 `promptBundleVersion`，以及非法的 `committedThroughSequence`、Memory/Model Context/Retrieval 协议和 revision 跨字段组合）再 encode 深复制；`restore`/decode 读取 `metadata.schemaVersion`，接受 v5/v6/v7/v8/v9，v5 缺失的提交边界映射为 `0`，旧协议缺失 Retrieval 时按 `none@1` 恢复，旧文件不改写且下一次正常保存生成 v9。v1 至 v4 与未知版本统一抛出 `INVALID_GOAL_SNAPSHOT`。并发写入为最后替换者覆盖，失败保存会清理 `.tmp` 临时文件，文件系统错误原样传播。
...
Runtime 执行协议按 Goal 冻结的 Prompt/Memory/Model Context/Retrieval 组合解码；本模块只负责 Snapshot 表示，不判断 Prompt Bundle 是否受 Agent 支持。v9 保存 `definition.memoryProtocol`、`definition.modelContextProtocol`、`definition.contextRetrievalProtocol` 与可选 `state.run.memoryRevision`，不保存 Working Memory 集合；v5–v8 缺失 Retrieval 时只读恢复为 `none@1`，新的 BM25-lite Goal 必须在写入前冻结完整协议。`state_committed` 只是 Snapshot 成功后的审计 marker，Working Memory 和后续 Retrieval 恢复消费者以 Snapshot 的 `committedThroughSequence` 和 revision 指针为权威边界；Sidecar 通过同一边界和来源摘要自校验，失配即可重建。后续新增 Bundle 版本不再升级 Snapshot Schema。演进背景见 [三视图分层架构 Spec](../../specs/three-view-architecture/design.md)。
