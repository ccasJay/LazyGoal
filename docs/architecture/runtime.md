...
Goal 将创建后冻结的 intent、`globalSystemPromptVersion`、Profile 和 executionPolicy 放在 `definition`，将 workflow、真实 messages 和 Run 放在 `state`。Runtime 只拥有 Global System Prompt 的版本标识，不持有或渲染文本；新 Goal 固定使用当前 v1，恢复后保留原版本。Run 可保存有界的 `checkpoint`、最近 `lastStep` 与当前 `pendingAction`；Action/Observation 不进入真实消息历史。新 Goal 从 `gathering_context/active` 与 `created/0` 开始；Preparation 不消费 Step，只有拥有最终 task 的 `executing` workflow 可进入 Runner。
