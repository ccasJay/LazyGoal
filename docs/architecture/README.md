...
LazyGoal 是一个 Goal 驱动的同步 Agent。`runtime` 拥有状态、生命周期、结构化 Working Memory 和持久化 Port，`storage` 提供 Snapshot v10 与 Profile 持久化，`agent` 通过 Projector 与 Renderer 把完整 Goal 转为一次模型调用，`llm` 隔离模型供应商。新 Goal 冻结 Prompt Bundle v7/`structured@1` Fact shape；v1–v3 checkpoint Goal 继续运行，v4–v6 structured Goal在 Memory 重建或模型调用前稳定拒绝。每个业务边界先提交事实与 canonical Memory Patch，再保存最新完整 Goal。
...
6. Coordinator/Runner 通过共享 Committer 追加事实与 canonical Patch，保存 Snapshot v10 的提交边界和 revision；Tool 返回后可由同步纯函数 Projector 提议 Fact，Runtime 统一做 evidence、去重、失效与容量准入。Snapshot 成功后才更新临时 Memory。
...
- 新 Goal 的 v7 Prompt Bundle 与 `structured@1` Fact shape 由 Composition Root 一起冻结；旧 v1–v3 Goal 按 `checkpoint@1` 兼容，v4–v6 structured Goal 返回 `UNSUPPORTED_STRUCTURED_MEMORY_SHAPE`。
