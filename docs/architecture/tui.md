...
`OpenAICompatible`、Profile Registry、`ReadFileTool`、`WriteFileTool`、`BashTool`、
`JsonFileGoalStore`、`CheckpointGateGoalStore`、Coordinator、Scheduler、Runner、根
`AbortController` 和 SessionController。Runner 注入 `createDefaultToolPolicy` 生成的
fail-closed 授权策略：只读 `read_file` 自动放行，`write_file`、`bash` 与任何未识别
Tool 都需要用户逐次批准。缺失或非法 Profile、未注册 Tool，以及缺失
